...
- Treat the browser-open todo as executable local work, not as a blocker. Emit a `local_tool_call` with `tool_name: bash` and `command: open <html_path>`, then complete the todo.
